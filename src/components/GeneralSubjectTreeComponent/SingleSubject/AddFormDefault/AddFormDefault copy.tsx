import { useState, useEffect, type ChangeEvent, type FormEvent } from "react";
import styles from "./AddFormDefault.module.scss";
import { validateDataArray, messagesVi } from "../../../../utils/validation";
import type { RuleSchema } from "../../../../utils/validation";
import { postData, deleteData, putData, getAuthHeaders } from "../../../../utils/axios/index";
import { AlertDialog, type AlertInfo } from '../../../../utils/AlertDialog';
import { v4 as uuidv4 } from 'uuid';
import checkFieldsAvailability from "../checkFieldsAvailability";


interface FieldConfig {
  label: string;
  type?: string;
  placeholder?: string;
  render?: (props: {
    value: any;
    onChange: (e: ChangeEvent<any>) => void;
    error?: string;
    name: string;
  }) => React.ReactNode;
}

interface SubjectManagementFormProps {
  urlCheck?: string;
  urlInsert?: string;
  fieldLabels: Record<string, FieldConfig>;
  ruleSchema: RuleSchema;
  checkFieldExists?: string[];
  checkFieldNoExists?: string[];
  activeData?: Record<string, any> | null;
  onSuccess?: (params: { action: 'insert' | 'update' | 'delete' | 'cancel', subject?: Record<string, any> }) => void;
  authorization?: { view?: boolean; add?: boolean };
  subjectName?: string;
}

export default function AddFormTreeDefault({
  urlCheck,
  urlInsert,
  activeData = null,
  onSuccess = () => { },
  authorization = {},
  fieldLabels = {},
  ruleSchema = {},
  checkFieldExists = [],
  checkFieldNoExists = [],
  subjectName = "",
}: SubjectManagementFormProps) {

  const [subjectData, setSubjectData] = useState<Subject>({});
  const [errors, setErrors] = useState<Partial<Record<keyof Subject, string>>>({});
  const [alertinfo, setAlertinfo] = useState<AlertInfo>({
    isAlertShow: false,
    alertMessage: '',
    type: 'error',
    title: 'Lỗi',
    showConfirm: true,
    showCancel: true
  });

  useEffect(() => {
    if (activeData) {
      const { _typeofRow, ...subjectWithoutTypeofRow } = activeData;
      setSubjectData({ ...subjectWithoutTypeofRow });
    } else {
      resetForm();
    }
  }, [activeData]);

  useEffect(() => {
    const timer = setTimeout(async () => {
      const newErrors = await checkFieldsAvailability({
        data: subjectData,
        urlCheck,
        fieldExists: checkFieldExists,
        fieldNoExists: checkFieldNoExists,
        idField: "id"
      });
      setErrors(prev => ({ ...prev, ...newErrors }));
    }, 300);
    return () => clearTimeout(timer);
  }, [subjectData]);

  const handleChange = (e: ChangeEvent<any>) => {
    const { name, value, type } = e.target;
    const val = type === 'checkbox' ? e.target.checked : value;

    setSubjectData(prev => ({ ...prev, [name]: val }));

    const singleFieldData = { [name]: val };
    const result = validateDataArray([singleFieldData], ruleSchema, messagesVi);

    setErrors(prev => ({
      ...prev,
      [name]: result.status ? "" : (result.results[0]?.errors?.[name] || "")
    }));
  };

  const validateForm = () => {
    const result = validateDataArray([subjectData], ruleSchema, messagesVi);
    if (!result.status) {
      setErrors(result.results[0]?.errors || {});
      return false;
    }
    return true;
  };

  const handleInsert = async (e: FormEvent) => {
    e.preventDefault();
    if (!validateForm()) return;

    const newErrors = await checkFieldsAvailability({
      data: subjectData,
      urlCheck,
      fieldExists: checkFieldExists,
      fieldNoExists: checkFieldNoExists,
      idField: "id"
    });


    if (Object.keys(newErrors).length > 0) {
      setErrors(prev => ({ ...prev, ...newErrors }));
      return;
    }
    console.log("subjectData", subjectData);
    setAlertinfo({
      isAlertShow: true,
      alertMessage: `Bạn có chắc chắn muốn tạo ${subjectName} mới?`,
      type: "warning",
      title: "Xác nhận",
      onConfirm: async () => {
        const newId = uuidv4();
        const subjectToCreate: Subject = { ...subjectData, id: newId };
        // loại bỏ key subRows nếu có
        Object.keys(subjectToCreate).forEach(key => {
          if (key.startsWith('subRows')) {
            delete subjectToCreate[key];
          }
        });
        const result = await postData({ url: urlInsert, data: subjectToCreate });
        console.log("result", result);
        if (result?.status) {
          setSubjectData(subjectToCreate);
          onSuccess?.({ action: "insert", subject: result.data });
        }
      },
      onCancel: () => setAlertinfo(prev => ({ ...prev, isAlertShow: false })),
      onClose: () => setAlertinfo(prev => ({ ...prev, isAlertShow: false })),
    });
  };

  const cancelForm = () => {
    resetForm();
    onSuccess?.({ action: "cancel", subject: subjectData });
  };

  const resetForm = () => {
    const emptyData: Subject = {};
    Object.keys(fieldLabels).forEach(field => {
      emptyData[field] = fieldLabels[field].type === "checkbox" ? false : "";
    });
    setSubjectData(emptyData);
    setErrors({});
  };

  if (Object.keys(fieldLabels).length === 0 || Object.keys(ruleSchema).length === 0) return null;

  return (
    <div className={styles.managementContainer}>
      <AlertDialog {...alertinfo} />
      {authorization.view && <h2 className={styles.title}>{`Thêm ${subjectName} mới`}</h2>}

      {authorization.add && (
        <form className={styles.subjectForm}>
          {Object.entries(fieldLabels).map(([field, config]) => (
            <div className={styles.formGroup} key={field}>
              <label htmlFor={field}>{config.label}:</label>
              {config.render ? (
                config.render({
                  value: subjectData[field as keyof Subject] || "",
                  onChange: handleChange,
                  error: errors[field as keyof Subject],
                  name: field
                })
              ) : config.type === "checkbox" ? (
                <input
                  type="checkbox"
                  id={field}
                  name={field}
                  checked={Boolean(subjectData[field as keyof Subject])}
                  onChange={handleChange}
                />
              ) : (
                <input
                  type={config.type}
                  id={field}
                  name={field}
                  value={subjectData[field as keyof Subject] || ""}
                  onChange={handleChange}
                  placeholder={config.placeholder}
                  className={errors[field as keyof Subject] ? styles.errorInput : ""}
                />
              )}
              {errors[field as keyof Subject] && (
                <span className={styles.error}>{errors[field as keyof Subject]}</span>
              )}
            </div>
          ))}

          <div className={styles.buttonGroup}>
            {authorization.add && (
              <button type="submit" className={styles.submitBtn} onClick={handleInsert}>
                Add
              </button>
            )}
            <button type="button" className={styles.cancelBtn} onClick={cancelForm}>
              Cancel
            </button>
          </div>
        </form>
      )}
    </div>
  );
}
